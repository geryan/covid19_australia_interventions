...
g_immunity_inheritance <- function(
  inheritance_type = c("infection", "vaccination", "vaccination_and_infection"),
  basic_types = c("S", "I"),
  vaccine_types = c("Az", "Pf"), # vector of possible vaccines
  vaccine_doses = 2#, # numeric or vector or length vaccine_types of number of doses of each vaccine
  # don't think we need this here # vaccine_switching = TRUE # is it possible to change from one vaccine to another
){
  type <- match.arg(type)
  
  n_vaccines <- length(vaccine_types)
  
  if(length(vaccine_doses) != n_vaccines){
    vaccine_doses <- rep(vaccine_doses, vaccine_types)
  }
  
  
  dose_number <- mapply(
    FUN = seq,
    from = rep(1, times = n_vaccines),
    to = vaccine_doses
  ) %>%
    unlist
  ...
  all_vaccines <- paste0(vaccines, dose_number)
  
  all_immunity <- bind_rows(
    tibble(
      from = basic_types,
      type = "basic"
    ),
...
create_transition_matrix <- function(
  ...
  # need to make this flexible to arbitrary immunity combinations
