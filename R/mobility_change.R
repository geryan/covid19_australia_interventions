...
for (this_state in all_states) {
  ...
  ggsave(
    filename = sprintf(
      "outputs/vl_half_ext/figures/%s_datastream_model_fit_%s.png",
      this_state,
      last_date
    ),
    width = 1500 / dpi,
...
mobility_fitted %>%
  ...
  saveRDS("outputs/vl_half_ext/google_change_trends.RDS")
...
ggsave(
  filename = sprintf(
    "outputs/vl_half_ext/figures/multistate_model_fit_%s.png",
    last_date
  ),
  width = 2481 / dpi,
  height = 3507 / dpi,
